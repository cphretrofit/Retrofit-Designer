...
### Phase 47 — Import/PDF speed + AONB heritage (2026-06 / this session)
- **Import no longer false-times-out**: `run_import_job` used to run 5–6 AI round-trips sequentially after the main extraction (template match → site-condition vision → design considerations → floor-plan detect [2 vision calls] → photo vision-tagging). These are now wrapped in coroutines and run **concurrently via `asyncio.gather`** (`_t_template`, `_t_site_and_considerations`, `_t_floorplan`, `_t_vision_tags`), then client-catalog + defect-match, then insert. Verified end-to-end: a 2-file real import finished in ~92s with ALL enrichment populated (9 design considerations, site conditions, template, 22 photos, 9 datasheet products) — nothing dropped.
- **Frontend import cap**: `ImportProject.jsx` poll cap raised 60→200 (~10 min) for 15-file jobs; the misleading "try smaller documents" toast replaced with an honest "still generating, check Projects list shortly" message.
- **PDF pack faster/reliable (direct download kept)**: `export_pack_pdf` now runs WeasyPrint render and `_collect_source_docs` **concurrently**; `_collect_source_docs` fetches all bound source docs from storage in **parallel** (was sequential get_object). Verified: 11MB (12 Marsh End, full appendix) and 10MB (Coldrush, 12 datasheets) packs both build in ~4–5s, HTTP 200, valid PDFs — well under the 120s gateway limit.
- **Heritage — AONB added**: the pack Designation Register table now includes **Area of Outstanding Natural Beauty (National Landscape)** and **National Park** rows (backend lookup + narrative statement already covered them). Verified with a Cotswolds postcode (GL54 2AN) → lookup returns `area-of-outstanding-natural-beauty`, pack table shows "Designation present" and the statement narrative cites the AONB/National Landscape.

### Phase 48 — Read full documents + real site-note defect photos (2026-06 / this session)
- **Root cause of "electric shower not mentioned" / missing defects**: `extract_pdf_text` only read the FIRST 8 PAGES and capped text at 11k chars. The SMART-EPC site notes are 29–30 pages — shower (p25), ventilation notes, and the whole Defects section (p25–30) were invisible to the AI. Fixed: `extract_pdf_text` default `max_pages` 8→40; per-doc `TEXT_LIMIT` default 11000→24000 (ASHP Survey 9000→20000, Datasheet stays 3000); OCR fallback capped at 12 pages. Verified: extracting the RdSAP site note now surfaces "Electric Shower", "Photo of shower", "Number of extract fans" (previously all False). NOTE: projects imported BEFORE this fix need a re-import (or re-extract) to pick up the shower — new imports get it automatically.
- **Site-note defect photos** (`extract_sitenote_defect_photos` + `_attach_sitenote_defect_photos` in ai_extractor.py): reads the structured "Defect N" blocks in surveyor site notes (SMART-EPC / SurveyorHub format), captures each defect's own "Defect photo:" images in reading order, and attaches them to the matching project defect. Matching is LOCATION-primary (site-note `Defect location:` vs the defect ELEMENT, not the full description — avoids "no fan in kitchen or WC" cross-matching) with a strong (≥3 word) description overlap as secondary; handles `br1`→`bedroom 1`. These are authoritative (`photoFromSiteNote`), never overridden by the fuzzy/AI matchers.
- **AI vision defect matcher** (`_ai_match_defect_photos`): for defects with no site-note photo, Claude vision reads the actual survey photos and assigns the best FIG (or clears a wrong AUTO match). Prioritises defect-relevant photos so bottom-of-document shots aren't skipped. Runs after the site-note attach + keyword match, in both the "Auto-match photos" button and import.
- Verified on live project 81 St Nicholas Road (8af609d2, 7 defects / 70 photos): after auto-match → Landing, Bedroom 1, WC, Living space get REAL site-note defect photos; Bathroom-fan, Windows, Internal-doors-undercuts get appropriate RdSAP form photos; zero wrong matches (Bathroom no longer grabs the Kitchen photo). Site-note photo downloads 200 image/jpeg. DefectsPanel renders `d.photo` unchanged; toast now counts site-note + keyword + AI matches.


### Phase 49 — Re-extract, defect galleries & auto-add (2026-06 / this session)
- **Re-read documents (existing projects)**: extended `reextract_project_fields` to also run `_attach_sitenote_defect_photos`; combined with the Phase 48 40-page read, the existing "Re-extract now" button (ReextractControl.jsx, in DesignWorkspace) now re-reads full documents and pulls site-note defect photos while preserving manual edits/measures/curation. VERIFIED end-to-end on 81 St Nicholas Road: `electric_shower` went from "not mentioned" (n/a) → `true` with citation "RdSAP site notes record 'Shower outlet type: Electric Shower'". Re-extract took ~185s (large full-doc Claude call + site-conditions vision + design considerations + site-note attach) — within the 240s UI poll window.
- **Defect galleries**: `_attach_sitenote_defect_photos` now stores ALL images per defect as `d.photos = [{url,caption}]` (primary in `d.photo`). DefectsPanel renders a thumbnail strip under the main photo; clicking a thumbnail sets it primary (updateField `defects.{idx}.photo`). Verified: Landing 2, Bedroom 1 & WC 3, Living space 2 photos.
- **Auto-add missing defects**: any logged site-note defect with no matching project defect is auto-added as a new defect (element = `Location — Defect type`, severity mapped from the site-note "Defect severity", `source:"sitenote"`). Verified: Kitchen — Wall Defects and Living Space — Wall Defect added (7→9 defects).
- **Idempotent**: each attached/added defect carries a `siteNoteKey`; re-runs skip already-applied site-note defects (RUN 2 added 0, count stayed 9). Applies to both the "Auto-match photos" button and re-extract.


### Phase 50 — Zoom, async PDF, condition proof photos, gallery captions (2026-06 / this session)
- **Defect photo zoom (lightbox)**: DefectsPanel main photo + gallery thumbnails open a full-screen lightbox (`data-testid="defect-lightbox"`) with prev/next, "Set as main", per-photo caption input, and close. Frontend-only; compiles clean.
- **Gallery captions**: each gallery photo now carries its OWN caption (edited in the lightbox), saved via `updateField` to `defects.{idx}.photos.{gi}.caption` (+ `photoCaption` when it's the primary). `defects.` is an allowed PATCH prefix.
- **Async PDF generation** (server.py): new `POST /projects/{id}/pack/generate` (background `_build_pack_job` via asyncio.create_task) → `GET /projects/{id}/pack/jobs/{job_id}` (status/progress/stage/ready) → `GET .../download`. Job renders + collects source docs concurrently, merges appendix, stores the PDF in object storage (`{APP_NAME}/packs/{job_id}.pdf`) in a `pack_jobs` collection. DesignPack.jsx now starts the job, polls, shows a progress bar (`pack-progress`, stages Reading→Typesetting→Merging→Finalising→Ready), then downloads — removes the 502 risk for very large packs. Verified: 8→45→100, download 200 (43MB pack). Old sync `pack.pdf` GET kept.
- **Site-condition proof photos**: `extract_sitenote_photo_labels` reads every embedded site-note photo with its nearest preceding label (e.g. "Photo of shower:"); `_attach_sitenote_condition_photos` maps labels→condition keys (shower/downlights/loft/floor/bathroom) and attaches an evidence image to any DETECTED condition lacking one. Wired into import, reextract, and the detect endpoint. SiteConditionsPanel + PDF already render `evidence[].url`.
- **Fixed latent bug**: the `/site-conditions/detect` endpoint ran pure-vision detection and DROPPED the doc-derived facts (so re-detecting wiped the shower read from the site-note text). It now calls `_merge_doc_site_facts` before attaching photos. Verified end-to-end on 81 St Nicholas Road: electric_shower present=True with evidence photo caption "Photo of shower" (source RdSAP site note); floor_type also gets a proof photo.


### Phase 51 — Floor-plan cleanup + loft insulation coverage (2026-06 / this session)
- **Overlapping rooms resolved** (`_resolve_overlaps` in cad_floorplan.py): rooms the AI traced on top of each other (e.g. Bed 1/Bed 2 stacked in one rectangle) are grouped by >35% overlap and split into their own halves so no two labels collide. User chose (b) — nudge geometry apart, not just labels.
- **Label overlaps fixed**: floor tag ("GF & FF") moved to the left margin above the left dimension; "Front Door" moved below the bottom dimension line (single line); multi-char cylinder labels (e.g. "HSI/C") drop below their box; right-column text (address, data-box title, notes, legend) is now word-wrapped to the column width via `_wrap` so nothing runs off the sheet edge (previously "First Floor: H 2.44m…" was cut off).
- **Dead space removed**: the signature/title block now sits directly under the PLAN (left of the divider), independent of the right-column height, and VB_H fits the taller stack — eliminating the large empty band under the plan.
