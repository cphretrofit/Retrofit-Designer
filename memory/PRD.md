...
## Ventilation — floor-plan pre-population + 3-state door undercut (Jun 2026)
- VentilationPanel now receives `floorPlan` prop (from DesignWorkspace). Internal Door Undercuts auto-populate from floor-plan room names (excludes circulation: hall/landing/corridor/lobby/stair/porch/entrance) when the list is empty; "Populate from plan" button on both the Undercut and Wet-Room schedules for non-destructive re-pull (wet-room button matches kitchen/bath/wc/en-suite/utility/shower/cloak/laundry).
- Door undercut select changed from boolean Yes/No to 3-state `status`: required ("Yes — required") | compliant ("Compliant") | none ("No door installed"). Back-compat: old `required` bool maps required=true→required, false→compliant. Save stores both `status` and synced `required`.
- Backend `pdf_builder._undercut_rooms` reads `status` (falls back to `required`); only `status=="required"` rooms are listed as needing an ADF1 para 1.25 undercut. Verified via UI: 4 rooms auto-loaded, dropdown shows the 3 options.

## Ventilation — wet-room auto-rates + undercut markers on plan (Jun 2026)
- "Populate from plan" on the Wet-Room Extract Schedule now pre-fills each new wet room's extract rate to the ADF1 Table 1.1 intermittent minimum (kitchen/utility/laundry 30 l/s, wc/toilet/cloak 6 l/s, bath/shower/en-suite 15 l/s), system "dMEV", note "ADF1 Table 1.1 minimum". Non-destructive (only adds rooms not already listed). Helper `wetRate()` in VentilationPanel.jsx.
- Internal-door undercut markers now render ON the floor plan (UI + PDF pack). server `_undercut_room_names(vent)` collects rooms with status=="required"; written into `cadData.undercutRooms` on both ventilation save (update_ventilation now re-renders cadSvg + clears packHash) and floorplan save (update_floorplan injects from current ventilation). `cad_floorplan._render_single` draws a teal "↧ UC" pill at the bottom-centre of each matching room and appends a legend line; `undercutRooms` propagated per-floor via `shared` keys. Persisting into cadData means every render call site (PDF builder, ai_extractor, GET rebuild) shows markers with no signature change.
- Verified live on 13 Mill View: saved undercuts (Bedroom/Living required, Bathroom compliant, Kitchen OP none) → cadData.undercutRooms=['Bedroom','Living'], 3 UC badges on plan (both bedrooms + living), compliant/none excluded; wet-room populate added Bathroom 15 l/s + Kitchen OP 30 l/s.

## Ventilation — undercut doorway air-gap line (Jun 2026)
- Undercut markers upgraded from a room-centre badge to a thin dashed teal air-gap line drawn ACROSS the doorway itself (with end ticks + small "UC" tag), for a clearer PAS/ADF1 detail. New `cad_floorplan._room_doorway(room, rooms)` finds the widest wall the required room shares with a circulation space (preferred) or any neighbour, returns (x, y, orient v/h, span capped 0.85m); the line is offset ~7px into the room and spans the door width. Falls back to the centre badge only when the room has no internal shared wall. Legend note updated to explain the dashed line = air gap under the door leaf.
- Verified live on 13 Mill View: 3 dashed lines + 3 UC tags for the 2 Bedrooms + Living; legend present in cadSvg (renders in UI + PDF pack).

## Readiness: clickable gaps + sign-off count fix + "Design sign-off" rename (Jun 2026)
- Clickable readiness: every "Project Readiness" gap row and its sub-items in IntelligencePanel.jsx are now buttons that call onOpen(section) — jump straight to the fix page (QA→Outstanding, Evidence claims→Site Conditions, Evidence datasheets→Evidence, others→their section). Added hover highlight + chevron. Helper gapTarget(bar,item). Verified: clicking Evidence gap navigated Outstanding→conditions.
...
## PDF export — survey routing, ventilation embedding, datasheet dedupe (Jun 2026)
Target mapping: Ventilation section → D1 table + ventilation strategy; Loft section → loft survey; Solar section → Easy PV report.
- Solar survey matching tightened (pdf_builder ~4510): new _is_solar_doc = positive PV/solar tokens (solar/pv/easypv/photovoltaic/mcs...) AND negative exclusions (loft, insulation, ashp, heat pump, ventilation, adf1). Stops a loft "technical survey" being picked as the solar survey. Used for both _solarSurveyMissing and the rasterise pick.
- Loft survey embedding: new p["_loftSurveyPages"] (rasterised via _pdf_to_page_uris) for LOFT measures, matched by loft/insulation/roof-space keywords with solar/ashp negatives; rendered inside the Loft spec section (mirrors solar/ashp survey blocks).
- Ventilation strategy + ADF1 Table D1 embedding: new p["_ventStrategyPages"] rasterised from a PDF doc_type Ventilation Strategy/ADF1/Air Tightness/etc matching adf1/table d1/ventilation strategy/checklist; injected right after ventilation_page. Generated _adf1_ventilation_pages now skipped when either _uploadedAdf1 OR embedded strategy pages exist (no duplication). NOTE: only PDFs rasterise — an .xlsx ventilation strategy won't embed as images.
- Phantom datasheet (JA Solar) + duplication fix (pdf_builder ~4436): _datasheetDocs deduped by normalised filename; client-library datasheets now only merged when the filename matches a product token actually specified on THIS project (manufacturer/product words), so unrelated library sheets no longer bind. If a phantom still appears it is in parsed datasheetProducts/measure.products (data-level) — needs the project ref to strip.
- Verified: syntax + import OK, backend restarts clean. Full-PDF verification pending on real project data (not in preview). REQUIRES REDEPLOY + re-export.

## PDF — drop AI-guessed products, go off uploaded datasheets (Jun 2026)
- Root cause of phantom "JA Solar": initial AI extraction seeds measure.products with a guessed panel (source=None); _assign_products only clears source=="datasheet" rows, so the guess survives even after real datasheets are parsed.
- Fix (render-time, pdf_builder._measure_datasheet_block): show ONLY datasheet-sourced products when any exist — `_sourced = [p in m.products if source=="datasheet"] + datasheet-matched prods`. If a datasheet is uploaded but not parsed to a product, show none (green "bound" badge). Only when NO datasheet exists at all do interim AI products still render. Products deduped by manufacturer|product. Appendix datasheetProducts list also deduped.
- Verified via unit test: JA Solar (no source) dropped when Astronergy datasheet present; interim product still shows when no datasheet. REQUIRES REDEPLOY.

## PDF export — root-cause fixes for duplication / phantom datasheet / missing D1 (Jun 2026)
Real cause was in _collect_source_docs (the actual Appendix-B PDF binder), not just the listing:
- Duplication (issue 4): it deduped only by storage_path, so the same panel/inverter datasheet uploaded to the project AND present in the client library (different paths) bound twice. Now deduped by NORMALISED FILENAME + storage_path. Verified: DUP filenames = [] across all 5 preview projects.
- Phantom JA Solar (issue 3): client-library datasheets were merged UNFILTERED. Now only bound if the filename contains a DISTINCTIVE product token (manufacturer/model) from a product specified on THIS project, with a generic-word stoplist (solar/module/panel/inverter/etc) so a loose "solar" match can't let a JA Solar library sheet through.
- Missing air-tightness / D1 (issue 2): _collect_source_docs only fetched doc_type "Datasheet", so the uploaded Ventilation Strategy / Air Tightness / ADF1 xlsx were NEVER bound. Now includes doc_type in [Datasheet, Ventilation Strategy, Air Tightness, ADF1]; LibreOffice (soffice, confirmed installed) converts the xlsx to PDF for Appendix B (verified: ADF1-Table-D1-.xlsx -> 835KB PDF).
