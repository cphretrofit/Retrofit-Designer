...
### Phase 23 — AI vision site-conditions (evidence-backed) + datasheet→products + Technical Surveys (2026-08-27)
- **AI vision site-condition detection** (Claude Sonnet 4.6 vision via emergentintegrations `ImageContent`): at import (and on-demand `POST /projects/{id}/site-conditions/detect`) the AI reads the survey photos + rasterised assessment/floor-plan pages and determines electric shower, bathroom floor, recessed downlights, floor type, loft cross-flow ventilation and loft storage. Every condition is **evidence-led** — it cites the survey-photo FIG that proves it (fig normalised + validated against the photo set; unmapped citations are cleared or labelled "Assessment / floor plan"), and stays null/"not visible" when there is no proof. Feeds `property.siteConditions` (drives the PAS 2035 compliance checklist incl. new loft cross-flow / boarding clauses).
- **Editable Site Conditions panel** (`SiteConditionsPanel`, workspace nav "Site Conditions"): review/override each verdict + detail with the evidence photo shown; Detect/Re-detect (with overwrite confirm) + Save. Pack renders "Section 01 · Site Conditions & Photographic Evidence".
- **Datasheets → Specified Products** (per-project, strictly isolated): `POST /projects/{id}/datasheets/parse` extracts manufacturer/product/ref/standard from uploaded datasheets and maps them to the matching measure's `products` (unmatched → project `datasheetProducts`); returns a count and 422 when nothing extractable. Products editable inline per measure; "Parse datasheets" button in Evidence. Pack renders per-measure Specified Products + "Appendix A · Product Datasheets & Supporting Documents".
- **Technical Survey** is now a first-class import doc type (extracted for content + photos + floor-plan rasterisation) and, with datasheets/other docs, listed in the Supporting Documents appendix so surveys are included in the issued pack.
- **Coldrush demo project** (RTF-2026-0166, id a9713cce-9685-4f8b-b300-c222a5903776): 12 real manufacturer datasheets (Ecodan ASHP + cylinder + controls, Fox ESS + DMEGC + battery, Knauf loft, Thermahood caps, Klober tile vent, Airbox trickle vent, Manthorpe felt lap vent, Vent-Axia Revive 7) parsed into 14 products across LOFT/ASHP/SOLAR/VENT. Script: `/app/backend/make_coldrush.py`.
- Verified: vision detect evidence-led (curl), pack.html/pdf render new sections (Coldrush 24pp), parse 422/count, testing_agent iteration_9 (34/34 backend, ~90% frontend) + follow-up fixes.

### Phase 24 — Clients + per-client datasheet library (2026-08-27)
- **Clients** are now a first-class entity (`clients` collection: id, name, status active/archived, products[]). Endpoints: `GET/POST /clients`, `PATCH /clients/{id}` (archive/restore/rename), `GET /clients/{id}`, `POST /clients/{id}/datasheets` (upload → rebuild catalogue), `DELETE /clients/{id}/datasheets/{doc_id}`. Seeded from existing project clients + Coldrush/Saffron/North Yorkshire.
- **New-design flow now starts with "Who is this design for?"** — the import page requires a client (pick or add inline) before uploading documents; client stored on the project.
- **Per-client datasheet library**: upload a client's datasheets once → parsed into that client's product catalogue (tagged by measure). Managed on the Clients page (click a client → `/clients/:id` detail with catalogue grouped by measure + datasheet list) and reflected during import. `documents` gained a `client_id`.
- **Auto-fill from library**: on import, after the AI reads the job card/scope and drafts measures, `_apply_client_catalog` fills each measure's Specified Products from the selected client's library (by measure code), strictly isolated per client. The per-project datasheet upload box was removed; Workspace → Evidence has an "Apply client library" button (`POST /projects/{id}/apply-client-library`) to re-sync. Datasheet product assignment is now idempotent (source tag + normalised dedupe — fixes iteration_10 accumulation).
- **Pack**: client-library datasheets are listed in the Supporting Documents appendix (`type = "Datasheet · <Client> library"`).
