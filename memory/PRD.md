...
- **#1 Datasheet supersedes brand:** EXTRACT_SYSTEM now has a "DATASHEET OVERRIDES BRAND" rule (a provided ISOVER datasheet supersedes a "Knauf" named in the Scope of Works). `_supersede_measure_brand` + `_INS_BRANDS` (incl. Knauf & Isover) already swap competitor brands across measure fields once the datasheet product is assigned. NOTE: full verify needs the ISOVER datasheet uploaded + applied to the project.
