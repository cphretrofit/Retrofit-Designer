...
## Recent changes
- 2026-06: Bay windows — click-to-place UX. "Add bay window" / "Add window" buttons enter placement mode; clicking the plan drops the window on the nearest wall (crosshair cursor + hint). Then drag the marker or edit via the "Windows & bays" list (type Box/Canted/Bow, wall, position, width, projection). Reusable `WindowsEditor` shown in both Visual & Rooms tabs. `FloorPlanGeometryEditor.jsx`.
