...
- SCOPE OF WORKS IS AUTHORITATIVE for which measures are actually installed. When a Scope of Works document is provided, include ONLY the measures its works sections confirm will be carried out. A measure that appears on the Job Card or assessment as recommended / considered / assessed but is NOT in the Scope of Works must be EXCLUDED (for example Solar PV shown on the Job Card but absent from the Scope of Works is NOT a measure — do not add it). If the Job Card and Scope of Works disagree on what is being installed, follow the Scope of Works.
- DATASHEET OVERRIDES BRAND: if a product datasheet is provided for a measure, that datasheet's manufacturer and product ARE the specified product and OVERRIDE any competitor brand named in the Scope of Works or assessment (e.g. a provided ISOVER datasheet supersedes a "Knauf" product named in the Scope of Works). Use the datasheet product in measures[].system and all specification text; never keep the superseded brand.
- LOFT / insulation DEPTH CONSISTENCY: state the existing depth, the top-up added and the total installed depth so they reconcile (total = existing + new). Use the total installed / target depth from the Scope of Works (e.g. "top-up to 300mm total"). NEVER state a total that contradicts the existing depth or exceeds the specified target — do not output 430mm when the Scope of Works specifies 300mm total. If the existing insulation already meets or exceeds the target depth, state that only a top-up / cross-lay to correct unevenness is required rather than adding a further full layer. The layers[] thicknesses for a loft measure must sum to the stated total installed depth.
